.class public Lcom/google/api/client/testing/json/webtoken/TestCertificates;
.super Ljava/lang/Object;
.source "TestCertificates.java"


# annotations
.annotation build Lcom/google/api/client/util/Beta;
.end annotation

.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;
    }
.end annotation


# static fields
.field public static final BOGUS_CA_CERT:Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;

.field public static final CA_CERT:Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;

.field public static final CA_KEY:Ljava/lang/String; = "-----BEGIN PRIVATE KEY-----\\nE+VTo/KjRA8P2ogks9bltd6f\n-----END PRIVATE KEY-----"

.field public static final FOO_BAR_COM_CERT:Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;

.field public static final FOO_BAR_COM_KEY:Ljava/lang/String; = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----"

.field public static final JWS_SIGNATURE:Ljava/lang/String; = ".eyJmb28iOiJiYXIifQ.eWzIsJF4PExQap9HK6Vlz8DGlgGwoiLCtyOEK0Bfu_yHTAZeApn5rh6Uzfx06Gv6eHdM34YL_tgLRb4bjuZVA8xvQ9uHNs8UtpBIOiUcagzvtKyyfCofk5U5sNb54GgVVYxa6p4A1ObdJv1jjlUOnzR8keX5LsAM4Ia7xeqiFh0GER4l0ulVChy_bSn0IeNiKFW7HKcxtcGO_zZTtlv4HiifuyPSk_ar2IDX1w599KXniVcWkQ_W1zcp5YuPDw8mIQDVCH2uQY7qs2ejdZj5LIgIz4CbQ0wg53rlwE7DDQM6MNUgZLnzNmMSMfFrpE7_PQyxe2qJCsucHODzEHX4Tg"

.field private static jsonWebSignature:Lcom/google/api/client/json/webtoken/JsonWebSignature;


# direct methods
.method static constructor <clinit>()V
    .locals 2

    .prologue
    .line 94
    new-instance v0, Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;

    const-string v1, "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"

    invoke-direct {v0, v1}, Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;-><init>(Ljava/lang/String;)V

    sput-object v0, Lcom/google/api/client/testing/json/webtoken/TestCertificates;->FOO_BAR_COM_CERT:Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;

    .line 154
    new-instance v0, Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;

    const-string v1, "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"

    invoke-direct {v0, v1}, Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;-><init>(Ljava/lang/String;)V

    sput-object v0, Lcom/google/api/client/testing/json/webtoken/TestCertificates;->CA_CERT:Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;

    .line 214
    new-instance v0, Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;

    const-string v1, "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"

    invoke-direct {v0, v1}, Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;-><init>(Ljava/lang/String;)V

    sput-object v0, Lcom/google/api/client/testing/json/webtoken/TestCertificates;->BOGUS_CA_CERT:Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;

    .line 294
    const/4 v0, 0x0

    sput-object v0, Lcom/google/api/client/testing/json/webtoken/TestCertificates;->jsonWebSignature:Lcom/google/api/client/json/webtoken/JsonWebSignature;

    return-void
.end method

.method public constructor <init>()V
    .locals 0

    .prologue
    .line 47
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 53
    return-void
.end method

.method public static getJsonWebSignature()Lcom/google/api/client/json/webtoken/JsonWebSignature;
    .locals 11
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    .prologue
    const/16 v10, 0x2e

    .line 297
    sget-object v8, Lcom/google/api/client/testing/json/webtoken/TestCertificates;->jsonWebSignature:Lcom/google/api/client/json/webtoken/JsonWebSignature;

    if-nez v8, :cond_0

    .line 298
    new-instance v2, Lcom/google/api/client/json/webtoken/JsonWebSignature$Header;

    invoke-direct {v2}, Lcom/google/api/client/json/webtoken/JsonWebSignature$Header;-><init>()V

    .line 299
    .local v2, "header":Lcom/google/api/client/json/webtoken/JsonWebSignature$Header;
    const-string v8, "RS256"

    invoke-virtual {v2, v8}, Lcom/google/api/client/json/webtoken/JsonWebSignature$Header;->setAlgorithm(Ljava/lang/String;)Lcom/google/api/client/json/webtoken/JsonWebSignature$Header;

    .line 300
    invoke-static {}, Lcom/google/api/client/util/Lists;->newArrayList()Ljava/util/ArrayList;

    move-result-object v0

    .line 301
    .local v0, "certificates":Ljava/util/List;, "Ljava/util/List<Ljava/lang/String;>;"
    sget-object v8, Lcom/google/api/client/testing/json/webtoken/TestCertificates;->FOO_BAR_COM_CERT:Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;

    invoke-virtual {v8}, Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;->getBase64Der()Ljava/lang/String;

    move-result-object v8

    invoke-interface {v0, v8}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    .line 302
    sget-object v8, Lcom/google/api/client/testing/json/webtoken/TestCertificates;->CA_CERT:Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;

    invoke-virtual {v8}, Lcom/google/api/client/testing/json/webtoken/TestCertificates$CertData;->getBase64Der()Ljava/lang/String;

    move-result-object v8

    invoke-interface {v0, v8}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    .line 303
    invoke-virtual {v2, v0}, Lcom/google/api/client/json/webtoken/JsonWebSignature$Header;->setX509Certificates(Ljava/util/List;)Lcom/google/api/client/json/webtoken/JsonWebSignature$Header;

    .line 304
    new-instance v3, Lcom/google/api/client/json/webtoken/JsonWebToken$Payload;

    invoke-direct {v3}, Lcom/google/api/client/json/webtoken/JsonWebToken$Payload;-><init>()V

    .line 305
    .local v3, "payload":Lcom/google/api/client/json/webtoken/JsonWebToken$Payload;
    const-string v8, "foo"

    const-string v9, "bar"

    invoke-virtual {v3, v8, v9}, Lcom/google/api/client/json/webtoken/JsonWebToken$Payload;->set(Ljava/lang/String;Ljava/lang/Object;)Lcom/google/api/client/json/webtoken/JsonWebToken$Payload;

    .line 306
    const-string v8, ".eyJmb28iOiJiYXIifQ.eWzIsJF4PExQap9HK6Vlz8DGlgGwoiLCtyOEK0Bfu_yHTAZeApn5rh6Uzfx06Gv6eHdM34YL_tgLRb4bjuZVA8xvQ9uHNs8UtpBIOiUcagzvtKyyfCofk5U5sNb54GgVVYxa6p4A1ObdJv1jjlUOnzR8keX5LsAM4Ia7xeqiFh0GER4l0ulVChy_bSn0IeNiKFW7HKcxtcGO_zZTtlv4HiifuyPSk_ar2IDX1w599KXniVcWkQ_W1zcp5YuPDw8mIQDVCH2uQY7qs2ejdZj5LIgIz4CbQ0wg53rlwE7DDQM6MNUgZLnzNmMSMfFrpE7_PQyxe2qJCsucHODzEHX4Tg"

    invoke-virtual {v8, v10}, Ljava/lang/String;->indexOf(I)I

    move-result v1

    .line 307
    .local v1, "firstDot":I
    const-string v8, ".eyJmb28iOiJiYXIifQ.eWzIsJF4PExQap9HK6Vlz8DGlgGwoiLCtyOEK0Bfu_yHTAZeApn5rh6Uzfx06Gv6eHdM34YL_tgLRb4bjuZVA8xvQ9uHNs8UtpBIOiUcagzvtKyyfCofk5U5sNb54GgVVYxa6p4A1ObdJv1jjlUOnzR8keX5LsAM4Ia7xeqiFh0GER4l0ulVChy_bSn0IeNiKFW7HKcxtcGO_zZTtlv4HiifuyPSk_ar2IDX1w599KXniVcWkQ_W1zcp5YuPDw8mIQDVCH2uQY7qs2ejdZj5LIgIz4CbQ0wg53rlwE7DDQM6MNUgZLnzNmMSMfFrpE7_PQyxe2qJCsucHODzEHX4Tg"

    add-int/lit8 v9, v1, 0x1

    invoke-virtual {v8, v10, v9}, Ljava/lang/String;->indexOf(II)I

    move-result v4

    .line 308
    .local v4, "secondDot":I
    const-string v8, ".eyJmb28iOiJiYXIifQ.eWzIsJF4PExQap9HK6Vlz8DGlgGwoiLCtyOEK0Bfu_yHTAZeApn5rh6Uzfx06Gv6eHdM34YL_tgLRb4bjuZVA8xvQ9uHNs8UtpBIOiUcagzvtKyyfCofk5U5sNb54GgVVYxa6p4A1ObdJv1jjlUOnzR8keX5LsAM4Ia7xeqiFh0GER4l0ulVChy_bSn0IeNiKFW7HKcxtcGO_zZTtlv4HiifuyPSk_ar2IDX1w599KXniVcWkQ_W1zcp5YuPDw8mIQDVCH2uQY7qs2ejdZj5LIgIz4CbQ0wg53rlwE7DDQM6MNUgZLnzNmMSMfFrpE7_PQyxe2qJCsucHODzEHX4Tg"

    add-int/lit8 v9, v4, 0x1

    invoke-virtual {v8, v9}, Ljava/lang/String;->substring(I)Ljava/lang/String;

    move-result-object v8

    invoke-static {v8}, Lcom/google/api/client/util/Base64;->decodeBase64(Ljava/lang/String;)[B

    move-result-object v6

    .line 309
    .local v6, "signatureBytes":[B
    const-string v8, ".eyJmb28iOiJiYXIifQ.eWzIsJF4PExQap9HK6Vlz8DGlgGwoiLCtyOEK0Bfu_yHTAZeApn5rh6Uzfx06Gv6eHdM34YL_tgLRb4bjuZVA8xvQ9uHNs8UtpBIOiUcagzvtKyyfCofk5U5sNb54GgVVYxa6p4A1ObdJv1jjlUOnzR8keX5LsAM4Ia7xeqiFh0GER4l0ulVChy_bSn0IeNiKFW7HKcxtcGO_zZTtlv4HiifuyPSk_ar2IDX1w599KXniVcWkQ_W1zcp5YuPDw8mIQDVCH2uQY7qs2ejdZj5LIgIz4CbQ0wg53rlwE7DDQM6MNUgZLnzNmMSMfFrpE7_PQyxe2qJCsucHODzEHX4Tg"

    const/4 v9, 0x0

    invoke-virtual {v8, v9, v4}, Ljava/lang/String;->substring(II)Ljava/lang/String;

    move-result-object v8

    invoke-static {v8}, Lcom/google/api/client/util/StringUtils;->getBytesUtf8(Ljava/lang/String;)[B

    move-result-object v7

    .line 310
    .local v7, "signedContentBytes":[B
    new-instance v5, Lcom/google/api/client/json/webtoken/JsonWebSignature;

    invoke-direct {v5, v2, v3, v6, v7}, Lcom/google/api/client/json/webtoken/JsonWebSignature;-><init>(Lcom/google/api/client/json/webtoken/JsonWebSignature$Header;Lcom/google/api/client/json/webtoken/JsonWebToken$Payload;[B[B)V

    .line 312
    .local v5, "signature":Lcom/google/api/client/json/webtoken/JsonWebSignature;
    sput-object v5, Lcom/google/api/client/testing/json/webtoken/TestCertificates;->jsonWebSignature:Lcom/google/api/client/json/webtoken/JsonWebSignature;

    .line 314
    :cond_0
    sget-object v8, Lcom/google/api/client/testing/json/webtoken/TestCertificates;->jsonWebSignature:Lcom/google/api/client/json/webtoken/JsonWebSignature;

    return-object v8
.end method
